...
class AttributesController extends Controller
{
    ...
    public function index(){
      try {
            ...
            $attributes = Attributes::where('lang', '=', $this->language)->orderBy('_id')->paginate(50);
    ...
    public function attributeSearch(){
      try {
        ...
        $attributes = Attributes::where('label', 'like', '%'.$term.'%')->orWhere('desc','like','%'.$term.'%')->where('lang','=', $this->language)->orderBy('_id')->paginate(50);
    ...
    public function autoAttrSearch() {
        try{
            if(Request::ajax()){
                $i = 0;
                $input = Request::all();       
                $attributes = Attributes::where('label', 'like', '%'.$input['term'].'%')->orWhere('desc','like','%'.$input['term'].'%')->where('lang', $this->language)->project( array('attr_id','label','desc'))->get();
    ...
    public function create(){
      try {
        ...
        $a_assocs= AttributesAssoc::where('lang', '=', $this->language)->get();//all(array('id','label'));
        $a_assocs_ = array();
        $a_types_ = array();
        foreach ($a_assocs as $a_assoc) {
            $a_assocs_[$a_assoc->id] = $a_assoc->id . ' - ' . $a_assoc->label;
        }        
        $a_types= AttributesType::where('lang', '=', $this->language)->get();//all(array('id','label'));
        foreach ($a_types as $a_type) {
            $a_types_[$a_type->id] = $a_type->id . ' - ' . $a_type->label;
        } 
        $dd_opts= DropdownOptions::where('lang', '=', $this->language)->get();
...
//            'len' => 'required|numeric|min:1',
        ...
        if ($validator->fails()) {
            ...
            if($attributes->save()){
                ...
                return redirect()->back()->with('message','Attribute could not be created. Please try Again.');
            }
        }
      }
      catch (\Exception $e){
    ...
    public function show($id){
      try {
            ...
            $attributes = Attributes::where('_id', '=', $id)->where('lang', $this->language)->first();
    ...
    public function edit($id){
      try {
            ...
            $attributes = Attributes::where('_id', '=', $id)->first();
            $a_assocs = AttributesAssoc::where('lang', '=', $this->language)->get();//all(array('id','label'));
            foreach ($a_assocs as $a_assoc) {
              $a_assocs_[$a_assoc->id] = $a_assoc->id . ' - ' . $a_assoc->label;
            }
...
//          'len' => 'required|numeric|min:1',
    ...
    public function assignCategories($id){
       try{
        if(!empty($assign_cat)){ 
            $categories= Categories::where('parent','=',0)->where('lang', $this->language)->where('slug','!=','')->get();
            $rules = array(
               'category' => 'required',
            );
