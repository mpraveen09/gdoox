    ...
    public function index(){
        try {        
            ...
            if(count($estores)>0) {
                ...
                $categories = Categories::where('lang', '=', $lang)->Where('parent', '=', 0)->orderBy('name')->project( array('cat_id', 'name') )->get();
            ...
            //        return;
                ...
                return redirect()->back()->withErrors("You don't have e-commerce site, so you can not add the product" );
            }
        }
        catch(\Exception $e){
...
//              $cat_list = $this->category_tree($input['parent']);
    ...
    function category_tree($parent, $term, $flag = FALSE , $user_tree_array = '',$last=0){
      try {
        ...
        $lang = session('app_language');
        if($flag === FALSE) {
            $sql = Categories::where('lang', '=', $lang)->Where('parent', '=', $parent)->orderBy('name')->project( array('cat_id', 'name') )->get();
                if(count($sql)){
                $user_tree_array[]= '<ul>';
                foreach ($sql as $row) {
        ...
        elseif($flag === TRUE) {
            $sql = Categories::where('lang', '=', $lang)
                ->Where('parent', '=', $parent)
                ...
                if(count($sql)){
                    ...
                    $other_ids = Categories::where('lang', '=', $lang)
                        ->Where('parent', '=', $parent)
                        ->orderBy('name')
                        ->project(array('cat_id', 'name'))
                        ->get();
    ...
    public function checkchild($other_ids, $parent, $term, $flag, $user_tree_array){
        ...
        foreach($other_ids as $val){
            $level = Categories::where('lang', '=', $lang)
            ...
            if(count($level)){
                ...
                $user_tree_array[] = '</ul>';
                 ...
                 $other = Categories::where('lang', '=', $lang)
    ...
    public function fetchCatAncestors(){
        try{
            if(Request::ajax()){
                ...
                $lang = session('app_language');
                do {
                    $sql = Categories::where('lang', '=', $lang)->Where('cat_id', '=', $currentid)->orderBy('name')->project( array('name','parent') )->first();//->take(1)->get();
                    if(count($sql)){
                        if($cat_hierachy === ""){
