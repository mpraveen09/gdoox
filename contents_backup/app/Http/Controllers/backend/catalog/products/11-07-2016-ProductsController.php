...
class _11_07_2016_ProductsController extends Controller
{
    ...
    public function changeProductVisibility($id){
        ...
        if($_GET['status'] ==='enabled'){
            if(empty($product->thumb) || empty($product->product_images)){
                $status = 'image_error';
            }
     ...
     * @return Response
     */    
    public function add(Request $request, $purpose){
...
//                exit;
                ...
                return view('backend.catalog.products.add', compact('productForm','purpose'));
            }
      }
      catch (\Exception $e){
          $error = "An error occured. ".
...
//        return;
        ...
        if(!empty($product) ){
         ...
         //Create Add Product form fields
          ...
          return view('backend.catalog.products.add', compact('productForm','flag'));
        }
        else {
...
//              $formFields[] = '</label></div></div>';
        ...
        foreach ($attrAssoc as $k=>$v) {
          ...
          foreach($attrForCats as $attrForCatk => $attrForCat){
                ...
                if(count($attributes)){
                    ...
                    elseif($productaction==='edit-product'){
                        ...
                        elseif($attr_id === '2' || $attr_id === 2){
                              ...
                              $formFieldsTemp[]= $this->createProductFormFields("attr_id[$attr_id]", $attributes->label, $attributes->desc, "TD", $attributes->len, "attr_".$attributes->class, $attributes->req,$siteslug, "",$disabled,'',$product,$attr_id);
                                ...
                                if($attr_id === '2' || $attr_id === 2){
                                    $siteslug = $this->getSites($userid);
                                    $formFieldsTemp[]= $this->createProductFormFields("attr_id[$attr_id]", $attributes->label, $attributes->desc, "TD", $attributes->len, "attr_".$attributes->class, $attributes->req,$siteslug, "",$disabled,'',$product,$attr_id, $purpose);
                                }
                                else {
                                      ...
                                      $fieldinfo .= "</i></small>";
                                      $formFieldsTemp[]= $this->createProductFormFields("attr_id[$attr_id]", $attributes->label, $attributes->desc, $attributes->field_type, $attributes->len, "attr_".$attributes->class, $attributes->req,$opt, "",$disabled, $fieldinfo, $product,$attr_id, $purpose);
                                }
                            }
               }                  
          }
    ...
    function createProductFormFields($attr_id, $label="", $desc="", $field_type="", $len="", $class="", $req="",$opt, $val="",$disabled="", $fieldinfo ="", $product="", $curr_attr="",$purpose=""){
...
//        echo "--------------------------------<br/>";
        ...
        switch ($field_type) {
            case "TD":
                if($purpose==='sell'){
                    if($attr_id !== 'attr_id[606]'){
                       // echo $purpose; echo "<br>"; echo $attr_id; exit;
      ...
      if(Auth::user()){
        try {
                ...
                foreach ($attrAssoc as $k=>$v) {
                    ...
                    return view('backend.catalog.products.ecosys_edit_product', compact('nav_menu','route','formFields','id','flag'));
        }
        catch (\Exception $e){
            $error = "An error occured. ".
                "Line Number: ".$e->getLine()." ".
...
//                $product->old_slug = $data['shopid'];
//                $product->desc = $data['desc'];
//                $product->shopid = $data['shopid'];
//                $product->postdate = $data['postdate'];
//                $product->thumb = $data['thumb'];
//                $product->thumb_path = $data['thumb_path'];
      ...
      if(Auth::user()){
        try {
                ...
                if($product){
                   ...
                   //Create Add Product form fields
                    $formFields = $this->createProductForm($prod_cats,"", $attrAssoc, $attrForCats, $userid, $product, $productaction);
//                    
//                    echo "<pre>";
//                    print_r($formFields);
//                    exit;
                    
                    return view('backend.catalog.products.edit_product', compact('formFields','id','flag','productaction'));
                }
...
//                
//                //Fetch Attributes
//                $ItemAttr = ProductHiddenAttributes::where('type','multiitem')->first();
//                $attrForCats = $ItemAttr->attr_ids;
//                sort($attrForCats);
//        
//                $formFields = array();
//                $formFields[] = $this->createProductFormFields("userid", "", "", "hidden", "", "", "","", Auth::user()->id);
...
//            }
            ...
            $productvars = Products::where('_id','=',$varid)->orWhere('product_variation_id','=',$varid)->where('status','=','enabled')->get();
            
            return view('backend.catalog.products.verify_variation',compact('productid','productvars','variationids','product'));
        }
        else {
            return redirect('auth/login')->with('message',"Please Login to Chat!");
         }
    ...
    public function createVariation(){
       ...
       $newproduct->product_images = $productdata->product_images;
...
//                                    <label for='seo_title' class='control-label col-md-3 col-sm-3 col-xs-12'>SEO Title</label>
//                                    <div class='col-md-6 col-sm-6 col-xs-12'>
//                                        <input type='text' id='seo_title' value='".$product->seo_title."' name='seo_title' class='form-control' placeholder='SEO Title'>    
//                                    </div>
//                                </div>
//
//                                <div class='form-group clearfix'>
//                                    <label for='seo_description' class='control-label col-md-3 col-sm-3 col-xs-12'>SEO Description</label>
//                                    <div class='col-md-6 col-sm-6 col-xs-12'>
//                                        <input type='text' id='seo_description' value='".$product->seo_description."' name='seo_description' class='form-control' placeholder='SEO Description'>  
//                                    </div>
//                                </div>
//
//                                <div class='form-group clearfix'>
//                                    <label for='seo_keywords' class='control-label col-md-3 col-sm-3 col-xs-12'>SEO Keywords</label>
//                                    <div class='col-md-6 col-sm-6 col-xs-12'>
//                                        <input type='text' id='seo_keywords' value='".$product->seo_keywords."' name='seo_keywords' class='form-control' placeholder='SEO Keywords'> 
//                                    </div>
//                                </div>
//                            <br />
//                       </div>";
...
//                    </div>';
//                    
//                    return view('backend.catalog.products.edit_product', compact('formFields','id','flag'));
//                }
