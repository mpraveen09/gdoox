    ...
    public function index(){
        try {
            if(Auth::user()){
              ...
              return view('backend.dashboard.users.index',compact('ab_cart_count','nav_menu','nav_menu','user_count','product_count','route'));
            }
            else {
            ...
            if(Auth::user()){
                ...
                foreach($roles as $role){
                    if(($role->level) > $rolelevel){
						...
						//changed by deep on 21 dec 2016
                        if($role->name !== 'multi-site-user'){
							$rolename[$role->name]=$role->name;
						}
                    }
                }  
				                
                $fm_data = FieldMaster::where('title', '=', 'register')->where('lang',$this->language)->first();
                $required = "*";
    ...
    public function show($id){
        try {
            if(Auth::user()){
                ...
                $rol = $this->getRoleName(Auth::user()->id);   
                $nav_menu = NavigationMenu::orderBy('sort_order','asc')->where('user', $rol)->where('menu', 'USER MANAGEMENT')->where('lang','en')->get();
                $route = Route::getCurrentRoute()->getName();
                
                return view('backend.dashboard.users.show',compact('user', 'role', 'level','fm_data', 'colleague','nav_menu','route'));
            }  
            else {
                 Session::flash('message', 'Please Login First');
                 return redirect('auth/login');         
    ...
    public function edit($id){
        try {
            if(Auth::user()){
               ...
               return view('backend.dashboard.users.edit',compact('user', 'userRole','rolename', 'level', 'fm_data', 'colleague','userrole','nav_menu','route'));
            }   
            else {
                Session::flash('message', 'Please Login First');
                return redirect('auth/login');         
    ...
    public function showAll(Request $request){
        try {
         if($this->hasRole('superadmin') || $this->hasRole('multi-user-admin') || $this->hasRole('multi-site-admin')){
            ...
            $companies = BusinessInfo::whereIn('user_id', $ids)->get();
            if(!empty($companies)){
                foreach($companies as $company){
                    $comp[$company->user_id] = $company->company_name;
                }
            }
...
//                if($interval < 7){
                  ...
                  $user->activation_key = '';
                   if($user->save()){
                    //added by deep on jan 16 2017
                    $roleID=$this->getRoleID('user');
                    $user_role = $this->roleID($user->_id);
                    if(!empty($user_role ) ){
                        $user_role = UserRole::where('user_id' ,'=', $user->_id)->first();
                        $user_role->role_id = $roleID;
                        if($user_role->save()){

                        }
                    }else{
                        $this->storeUserRole($user->_id, $roleID);
                    }
...
//               return redirect('auth/login')->with('message','You have already Activated your Account. Please Login with the Registered Email Id and Password!');
    ...
    public function allow($id){
        try {
                ...
                $labels = TeamMemberPermissionFields::where('type','permissions')->where('lang',$this->language)->get();
                $labelsmain = TeamMemberPermissionFields::where('type','permission_group')->where('lang',$this->language)->get();
    ...
    public function manageTeamMemberPermissions(Request $request, $id){
        if(Auth::user()){
            ...
            return view('backend.dashboard.users.manage_permissions',compact('user','fm_data','personalSites', 'ecomsite', 'permission', 'temp', 'perm','toplevel','sublevel','id','assigned','ecomsites','assigned_crm_permissions','assigned_permissions','nav_menu','route'));
    ...
    public function getRoutes($permissions){
        $permroutes = TeamMemberPermissionFields::whereIn('name', $permissions)->where('lang', $this->language)->get();  
        $routes = array();
 ...
 */
    ...
    public function allGdooxMember(){
      if(Auth::user()){
        try {
                $users = $roles = $allowedby = $createdby = array();
                $fm_data = FieldMaster::where('title', '=', 'register')->where('lang', $this->language)->first();
    ...
    public function viewGdooxMember(Request $request){
        if(Auth::user()){
            $data = $request->all();
            $id = $data['id'];
            $member = User::where('_id', $id)->first();
            $fm_data = FieldMaster::where('title', '=', 'register')->where('lang', $this->language)->first();
            ...
            $rol = $this->getRoleName(Auth::user()->id);
            $nav_menu = NavigationMenu::orderBy('sort_order','asc')->where('user',$rol)->where('menu', 'USER MANAGEMENT')->where('lang', $this->language)->get();
            $route = Route::getCurrentRoute()->getName();
            
            return view('backend.dashboard.users.view_gdoox_member',compact('role','level','fm_data','member','nav_menu','route')); 
        }
        else {
    ...
    public function editGdooxMember(Request $request){
        if(Auth::user()){
            $rolename = array();
            $data = $request->all();
            ...
            foreach($roles as $val){
                if(($val->level) > Session::get('level')){
                    $rolename[$val->name] = $val->name;
                }
            }
            
            
            return view('backend.dashboard.users.edit_gdoox_member',compact('role','level','fm_data','user','rolename','nav_menu','route'));
        }
        else {
            return redirect('auth/login')->with('message', 'Please Login');
        }
    ...
    public function updateGdooxMember(Request $request){
        if(Auth::user()){
                $request->all();                
                $id = $request['id'];
                ...
                $user = array('email'=>$request->input('email'),'username'=>$request->input('name'),'password'=>$password);
                User::where('_id', $id)->update($user, array('upsert' => true));
                
                return redirect()->route('gdoox_member.view_all')->with('message','Gdoox User has been updated successfully');
        }
        else {
            return redirect('auth/login')->with('message', 'Please Login');
        }
    ...
    public function managePermisson($id){
      if(Auth::user()){
        try {
                ...
                return view('backend.dashboard.users.manage_permission',compact('user', 'fm_data', 'permission','nav_menu','route'));
        }
        catch (Exception $ex) {
            $errors = $this->errorMessage($e);
            return Redirect::route('exceptions.create')->with('message','Oops! You are here because Something went Wrong.  Please report the Error')->withErrors($errors);
    ...
    public function InviteColleague() {
        if(Auth::user()){ 
            try {
                ...
                $role = $this->getRoleName(Auth::user()->id);
                $nav_menu = NavigationMenu::orderBy('sort_order','asc')->where('user',$role)->where('menu', 'MANAGING ACCOUNT')->where('lang','en')->get();
                $route = Route::getCurrentRoute()->getName();
    ...
    public function EditColleague(Request $request){
        try {
           if(Auth::user()){
              ...
              return view('backend.dashboard.users.edit_colleague',compact('user', 'userRole','rolename', 'level', 'fm_data', 'colleague','userrole','nav_menu','route'));
           }   
           else {
               Session::flash('message', 'Please Login First');
               return redirect('auth/login');         
