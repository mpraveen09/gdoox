...
class CategoriesController extends Controller {
    ...
    public function index(){
        try{
            ...
            $categories = Categories::where('lang', '=', $lang)->orderBy('cat_id')->paginate(50);
    ...
    public function categorySearch(){
        try {
          $term = Request::input('term');
          $lang = session('app_language');
          $categories= Categories::where('lang', '=', $lang)->where('name', 'like', '%'.addslashes($term).'%')->orderBy('cat_id')->paginate(50);
    ...
    public function autoCatSearch() {
        try {
            if(Request::ajax()){
                $input = Request::all();
                $lang = session('app_language');
                $categories= Categories::where('name', 'like', '%'.$input['term'].'%')->where('lang', '=', $lang)->project( array('cat_id','name'))->get();
                $i = 0;
                $response = array();
    ...
    public function create(){
      try {
            ...
            $lang = session('app_language');
            $categories_= Categories::where('lang', '=', $lang)->where('leaf', '!=', '1')->where('leaf', '!=', 1)->orderBy('name')->get();
      ...
      try {
        ...
        if ($validator->fails()) {
            ...
            if($categories->save()){
                ...
                return redirect()->back()->with('message','Category could not be created. Please try Again');
            }
        }
      }
      catch (\Exception $e){
      ...
      try{
        ...
        if ($validator->fails()) {
            ...
            if($categories->save()){
                ...
                return redirect()->back()->with('message','Attribute could not be updated. Please try Again.');
            }
        }
      }
      catch (\Exception $e){
...
//    }
    ...
    public function attrSearch() { 
      try {
        ...
        $attributes= Attributes::where('label', 'like', '%'.$term.'%')->orWhere('desc','like','%'.$term.'%')->where('lang', '=', $lang)->orderBy('_id')->get();
     ...
     public function assignAttributes($id){
       try {
            $role = $this->getRoleName(Auth::user()->id); 
            $lang = session('app_language');
            $nav_menu = NavigationMenu::orderBy('sort_order','asc')->where('menu','GDOOX PLATFORM MANAGEMENT TEAM')->where('user',$role)->where('lang', $lang)->get();
            $route = Route::getCurrentRoute()->getName();
            ...
            if(!empty($assign_attr)){
                ...
                $attributes= Attributes::orderBy('_id', 'asc')->where('lang', '=', $lang)->where('label','!=','')->get();
                if($values->save()) {
                    $attr= array();
                    $new_values = CategoryAttribute::where('cat_id','=',$id)->first();
                    if(!empty($new_values)){
                    ...
                    Session::flash('message', 'Category assigned to Selected Attributes Successfully');
                    return view('backend.catalog.categories.assign_attr')
                            ->with(compact('attributes','attr','nav_menu','route'))->with('id',$id); 
                }
                else {
                        $attr= array();
                        $new_values = CategoryAttribute::where('cat_id','=',$id)->first();
                        ...
                        return view('backend.catalog.categories.assign_attr')->with(compact('attributes','attr','nav_menu','route'))->with('id',$id); 
                }
