...
//                        return Redirect::route('ecosys.product.index',['site_slug'=>$_GET['site_slug']])->with('message', "Product has been ".$_GET['status']);                    
//                    }else{
//                        return Redirect::route('products/procurements/{purpose}',['site_slug'=>$_GET['site_slug']])->with('message', "Product has been ".$_GET['status']);
//                    }
                }
...
//                        return Redirect::route('products/list')->with('message', "Product has been ".$_GET['status']);
//                    }else{
//                        return Redirect::route('products/procurements/',['purpose'=>'purpose'])->with('message', "Product has been ".$_GET['status']);
//                    }
    ...
    public function changeProductVisibility($id){
        ...
        if($_GET['status'] ==='enabled'){
            if($product->purpose === 'sell'){
                ...
                elseif(empty($product->product_data[8])){
                    $status = 'quantity_error';
                }
                elseif(empty($product->product_data[16]) && empty($product->product_data[18]) && empty($product->product_data[20]) && empty($product->product_data[21])){
                     $status = 'price_error';
                }
                else {
                ...
                if(empty($product->product_data[8])){
                    //$status = 'quantity_error';
                    //commented for procurement products, verify later and change this accordingly
                    DB::collection('products')->where('_id', $id)->update(['status'=>$_GET['status'], 'disable_edit'=>'Yes']);
                    $status = 'success';                    
                }
                elseif(empty($product->product_data[22])){
                     $status = 'price_error';
                }
                else {
                    DB::collection('products')->where('_id', $id)->update(['status'=>$_GET['status'], 'disable_edit'=>'Yes']);
...
//    if(Auth::user()){ echo strtoupper(Auth::user()->id);}   
      try {
            ...
            if ($validator->fails()) {
                return Redirect::route($rt)->withErrors($validator);                        
            } else {
                ...
                //Added on 17 Jan 2017 by Deep
                //because the atrr 606 was not added to cat attribute relation table
                if($purpose === 'buy'){
                    $attrForCats['606'] = 606;
                }               
...
//                exit;
                ...
                return view('backend.catalog.products.add', compact('productForm','purpose','classification','nav_menu','route'));
            }
      }
      catch (\Exception $e){
          $error = "An error occured. ".
...
//                   }  
                    ...
                    if($type=='temporary' && $type!=''){
                        ...
                        elseif($attr_id === '2' || $attr_id === 2){
                              ...
                              $formFieldsTemp[]= $this->createProductFormFields("attr_id[$attr_id]", $attributes->label, $attributes->desc, "TD", $attributes->len, "attr_".$attributes->class, $attributes->req,$siteslug, "",$disabled,'',$product,$attr_id);
                                  ...
                                  //field type and length
                                  $a_type = AttributesType::where('id', '=', $attributes->field_type)->where('lang', '=', $this->language)->project(array('label'))->first();
                                  $fieldinfo = "<br/><small><i>";
                                  $fieldinfo .= $a_type->label;
                                  if(!empty($attributes->len)){
                                        $fieldinfo .= " (Length ". $attributes->len . ")";
    ...
    function createProductFormFields($attr_id, $label="", $desc="", $field_type="", $len="", $class="", $req="",$opt, $val="",$disabled="", $fieldinfo ="", $product="", $curr_attr=""){
        ...
        $countries = DropdownOption::where('name','countries')->where('lang', $this->language)->get();
...
//                    </div>";                    
    ...
    public function store(Request $request){
      try {
            ...
            $price_attrs = array("15","16","17","18","19","20","21","22","26","27","28","29","29a","30","31","32","33","357","358");
            ...
            foreach($input['attr_id'] as $attr_id => $attr_val){
                if(gettype($attr_val) === "object"){
                    ...
                    if(in_array($attr_id, $price_attrs)){
                        $product_attr[$attr_id] = (double)$attr_val;
                    }
                    else {
...
//            exit;
            ...
            if($product->save()){
                ...
                $inventory = $this->saveProductInventory($productids);
                if($matchedids >= 1){
...
//        }
    ...
    public function updateSharedProduct($id){
      if(Auth::user()){
        try {
                $price_attrs = array("15","16","17","18","19","20","21","22","26","27","28","29","29a","30","31","33","357","358");
                $data = Request::all();
    ...
    public function editProduct($id){
      if(Auth::user()){
        try {
                ...
                if($type==='temporary'){
                    return view('backend.catalog.products.edit_variation_product', compact('formFields','id','flag','productaction','purpose','product','type','nav_menu','route'));
                }
                else {
                    return view('backend.catalog.products.edit_product', compact('formFields','id','flag','productaction','purpose','product','type','nav_menu','route'));
                }
        }
        catch (\Exception $e){
            $error = "An error occured. ".
    ...
    public function updateProductInfo($id){
      if(Auth::user()){
        try {
                ...
                if($product->save()){
                    $inventory = $this->saveProductInventory($product);
...
//             exit;
    ...
    public function verifyProductVariation($id){
      if(Auth::user()){
              ...
              if($save==='success'){
                  ...
                  foreach($attrForCats as $attrForCatk => $attrForCat){
                      $attributes = Attributes::where('attr_id', strval($attrForCat) )->where('lang', '=', $this->language)->first();
    ...
    public function saveProductInventory($product_ids ){
	if(!empty($product_ids)){
            $inventory = ProductInventory::where('product_id', $product_ids->_id)->first();
            if(empty($inventory)){
                if(isset($product_ids->product_data['8'])){
                    $productinventory = new ProductInventory();
                    $productinventory->product_id = $product_ids->_id;
                    $productinventory->user_id = Auth::user()->id;
                    $productinventory->total_products = (double)$product_ids->product_data['8'];
                    $productinventory->remaining_products = (double)$product_ids->product_data['8'];
                    $productinventory->sold_products = '';
                    $productinventory->save();
                }
            }
            else {
                $inventory->total_products = (double)$product_ids->product_data['8'];
                $inventory->remaining_products = (double)$product_ids->product_data['8'];
