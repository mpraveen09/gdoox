        ...
        <div class="col-md-12">
          <div class="card">
              <div class="card-header card-padding-sm bgm-blue">
                  <h2>{!! $fm_data->labels['language'] !!}</h2>
              </div>
              
              <div class="card-body card-padding">
                    <div class="table-responsive" tabindex="1" style="overflow: hidden; outline: none;">
                        @if($sitetype==='personal')
                        @if($storename->professional_skills['mother_tongue'])
                            <table class="table table-striped responsive-utilities jambo_table ">
                                 <tbody> 
                                        <tr>
                                            <td class="col-md-4">{!!$fm_data->labels['mother_tongue']!!}</td>
                                            <td>{!! $storename->professional_skills['mother_tongue'] !!}</td>
                                        </tr>
                                  </tbody>
                            </table>
                        @else
                        ...
                        @endif
                         ...
                         @if($storename->professional_skills['other_languages'])
                            <table class="table table-striped responsive-utilities jambo_table ">
                                <thead>
                                    <th>Language</th>
                                    <th>Speaking</th>
                                 ...
                                 <tbody>
                                        @foreach($storename->professional_skills['other_languages'] as $val)
                                            <tr>
                                                <td>{!! $val['language'] !!}</td>
                                                <td>{!! $val['speaking'] !!}</td>
                                                <td>{!! $val['writing'] !!}</td>
                                        ...
                                        @endforeach
                                        @foreach($storename->professional_skills['languages'] as $val)
                                            <tr>
                                                <td>{!! $val['language'] !!}</td>
                                                <td>{!! $val['speaking'] !!}</td>
                                                <td>{!! $val['writing'] !!}</td>
