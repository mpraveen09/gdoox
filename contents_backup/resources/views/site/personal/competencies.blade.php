      ...
      <div class="row">
        <div class="col-md-12">
          <div class="card">
              ...
              @if($storename->competencies)
              <div class="card-body card-padding">
                <div class="table-responsive" tabindex="1" style="overflow: hidden; outline: none;">
                     <table class="table table-striped responsive-utilities jambo_table ">
                         <thead>
                            <th>{!!$fm_data->labels['competencies']!!}</th>
                            <th>{!!$fm_data->labels['competencies_tags']!!}</th>
                            <th>{!!$fm_data->labels['short_description']!!}</th>
                            <th>{!!$fm_data->labels['extensive_description']!!}</th>
                         </thead>
                          <tbody>
                            @if($sitetype==='personal')
                               @foreach($storename->competencies as $value)
                                    <tr>
                ...
                @if(Auth::user())
                  @if(Auth::user()->id==$personalsitedetails->user_id)
                    <div class="card-body card-padding">
                        <a href="{!!route('competencies-create')!!}" type="submit" class="btn btn-default">Edit</a>
                    </div>
                  @endif
                @endif
              </div> 
              ...
              <div class="card-body card-padding">
                <div class="table-responsive" tabindex="1" style="overflow: hidden; outline: none;">
                  ...
                  @if(Auth::user())
                    @if(Auth::user()->id==$personalsitedetails->user_id)
                      <div class="card-body card-padding">
                          <a href="{!!route('competencies-edit',$personalsitedetails->user_id)!!}" type="submit" class="btn btn-default">Edit</a>
                      </div>
                    @endif
                  @endif
              </div> 
