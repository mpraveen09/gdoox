        ...
        try {
          ...
          if(isset($_GET['product_id'])){
                ...
                foreach ($categories as $category){
                      $cat['cat_id'][] = $category;
                      $cat_category =  Categories::where('cat_id', $category)->first();
                ...
                foreach($product->product_data as $key=>$value){
                      $attr['attr_id'][] = $key;
                      $attribute =  Attributes::where('attr_id', strval($key))->first();
                      if(!empty($attribute))
              ...
              if(count($attributes)){
                ...
                if($attr_id === '2' || $attr_id === 2){
                  ...
                  if($attributes->field_type === "TD" || $attributes->field_type === "TM" ){
                      $dropopt =  DropdownOptions::where('attr_id', $attributes->attr_id )->where('lang', '=', $this->app_language)->first();
                      if(count($dropopt )){
