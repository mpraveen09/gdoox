    ...
    public function products(){
      if(Auth::user()){
        try{
            ...
            return view('backend.catalog.products.opportunities.products', compact('products'));
        }
        catch (\Exception $e){
            $error = "An error occured. ".
                            "Line Number: ".$e->getLine()." ".
    ...
    public function manage($id){
        if(Auth::user()){
          try {
            ...
            return view('backend.catalog.products.opportunities.manage', compact('product', 'attributes'));
          }
          catch (\Exception $e){
              $error = "An error occured. ".
                              "Line Number: ".$e->getLine()." ".
        ...
        foreach($attributes as $attrib){
            if($attrib->attr_id != 78){
                ...
                if($attrib->field_type === "TD" || $attrib->field_type === "TM" ){
                    $dropopt =  DropdownOptions::where('attr_id', $attrib->attr_id )->where('lang','=',$this->language)->first();
                    if(count($dropopt )){
