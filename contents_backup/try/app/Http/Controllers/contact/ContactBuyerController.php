...
class ContactBuyerController extends Controller {
    ...
    public function index(){
        if(Auth::user()){
            ...
            $messages = ComplaintMessages::groupBy('user_id')->orderBy('created_at','=','desc')->whereIn('store',$mystores)->where('from','=','buyer')->paginate($limit);
            $update = AlertSystem::where('user_id','=',Auth::user()->id)->first();
            $update->buyer_messages = array('read_at'=>date("Y-m-d H:i:s"),"notification"=>"");
            $update->save();
            
            return view('contact.contact-buyer.index',  compact('messages'));
        }
        else {
