...
class UserOrdersController extends Controller
{
    public function index(){
        if(Auth::user()){
            try {                 
                ...
                $nav_menu = NavigationMenu::orderBy('sort_order','asc')->where('parent', '56')->where('group','orders')->where('lang','en')->get();
                ...
                $tracking = OrderTracking::whereIn('sub_order_id', $subOrderIds)->where('customer_id','=',Auth::user()->id)->project('sub_order_id','product_status','status_log','status')->get();
                return view('userorders.index', compact('nav_menu','route','orders','tracking'));
            }
            catch (\Exception $e){
    ...
    public function returnProduct(){
        if(Auth::user()){
            ...
            $returnreasons = DropdownOption::where('name','=','Return Reasons')->where('lang','=','en')->first();
            foreach($returnreasons->options as $key=>$val){
                $reasons[$val] = $val;
            }
 
            return view('userorders.return_product', compact('reasons','product','returnstatus'));
