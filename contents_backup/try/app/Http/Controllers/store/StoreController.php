...
class StoreController extends Controller
{
    ...
    public function index(){
        try {
                ...
                $products_list = array();
                $country = array();
                $projections = array('cat_ids', 'shopid', 'desc', 'postdate', 'product_data', 'thumb', '_id', 'thumb_path','product_images');
...
//                exit;
                ...
                if(!empty($term)){
                    ...
                    $option =  DropdownOptions::where('attr_id','=',14 )->where('lang', '=', $this->language)->first();
                    if(empty($action)){
                    ...
                    if(!empty($term['sale_type'])){
                        if($term['sale_type']==='Price - Buy'){
                            $builder->where('product_data.14','=','Price - Sell');//->orWhere('product_data.14','=','Price - For Rent');
                        }
                        elseif($term['sale_type']==='Price - Sell'){
                            $builder->where('product_data.14','=','Price - Buy');//->orWhere('product_data.14','=','Price - To Rent');
                        }
                        elseif($term['sale_type']==='Auction'){
                            $builder->where('product_data.14','=','Auction');
                        }
                        elseif($term['sale_type']==='Reverse Auction'){
                            $builder->where('product_data.606','=','Reverse Auction');
                        }
                    }
           ...
           // $currency= array();
            ...
            return view('store.index', compact('products', 'products_counts', 'products_list', 'filtered_cat', 'com', 
                  'country', 'currency', 'maximum_product_price', 'minimum_product_price','min_price','max_price'))
    ...
    public function siteHome($shopid) {
        try {
            ...
            $products_list = array();
            $filtered_cat = array();
            $projections = array('cat_ids', 'desc', 'postdate', 'product_data', 'thumb', '_id', 'thumb_path','product_images');
...
//            exit;
        ...
        return view('site.index', compact('sitesubuser', 'subid', 'site', 'subuser', 'owner', 'site_menu', 
                ...
                'ecommerce_sites','eco_system_sites','network_site','user_sites'))
            ...
            foreach($attrForCats as $attrForCatk => $attrForCat){
                ...
                if(count($attributes)){
                    ...
                    if($attributes->field_type === "TD" || $attributes->field_type === "TM" ){
                        $dropopt =  DropdownOptions::where('attr_id', (int)$attributes->attr_id )->where('lang', '=', 'en')->first();
                        if(count($dropopt )){
