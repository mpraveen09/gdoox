        ...
        try {
            if(Auth::guest() || Auth::user()){
                if(Auth::user()){
                    ...
                    $nav_menu = NavigationMenu::orderBy('sort_order','asc')->where('user',$role)->where('menu', 'SEARCH COMPANIES/SITES')->where('lang','en')->get();
...
//                    }
                     ...
                     return view('backend.dashboard.search_business_info.index',compact('fm_data','business_info','term','category','search_val','nav_menu','route'));
                  // return view('backend.dashboard.search_business_info.index',compact('fm_data'))->with('business_info',$business_info)->with('term',$term)->with('category',$category)->with('estore',$estore);
                    ...
                    $term['term']='0';
                    $search_val= '';
                    return view('backend.dashboard.search_business_info.index',compact('fm_data','category','term','search_val','nav_menu','route'));
                }
             }
             else {
    ...
    public function show($id){
        try {
            if(Auth::user()){
                ...
                return view('backend.dashboard.search_business_info.show',compact('business','fm_data','nav_menu','route'));
            }
            else {
