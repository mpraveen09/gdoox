...
class DistributionNetworksController extends \Gdoox\Http\Controllers\backend\FieldMasterController {
    ...
    public function index() {
        try {
            if(Auth::user()){
                ...
                $nav_menu = NavigationMenu::orderBy('sort_order','asc')->where('user',$role)->where('menu', 'DISTRIBUTION NETWORK')->where('lang','en')->get();
                ...
                return view('distribution_network.index',  compact('types','network_types','route','nav_menu'));
    ...
    public function create() {
        try{
            ...
            $network_type = DropdownOption::where('name','=','Distribution Network')->first();
            ...
            return view('distribution_network.create',  compact('fm_data','terms','network_type','types','country','tc','nav_menu','route'));
        }
        catch(\Exception $e){
            $errors = $this->errorMessage($e);
            return Redirect::route('exceptions.create')->with('message','Oops! You are here because Something went Wrong.  Please report the Error')->withErrors($errors);;
...
//                    $network->t_n_c_3 = $data['t_n_c_3'];
    ...
    public function edit($id) {
        try {
            if(Auth::user()){
                ...
                $terms = TermsAndCondition::where('title','=','personal_info')->where('lang', $this->language)->first();
                ...
                return view('distribution_network.edit',compact('fm_data','terms','networks','types','country','tc','nav_menu','route'));
            }
            else {
    ...
    public function show($id){
        try {
            ...
            $tc = TermsAndCondition::where('title', 'personal_info')->where('lang', 'en')->first();
            return view('distribution_network.show',compact('fm_data','network','tc','nav_menu','route'));
        }
        catch(\Exception $e){
            $errors = $this->errorMessage($e);
            return Redirect::route('exceptions.create')->with('message','Oops! You are here because Something went Wrong.  Please report the Error')->withErrors($errors);;
    ...
    public function viewNetwork($val) {
       try {
          ...
          return view('distribution_network.view_network',  compact('networks','nav_menu','route'))->with('type',$type)->with('term',$term);
       }
       catch(\Exception $e){
          $errors = $this->errorMessage($e);
          return Redirect::route('exceptions.create')->with('message','Oops! You are here because Something went Wrong.  Please report the Error')->withErrors($errors);;
