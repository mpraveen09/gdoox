...
class SubscriptionGdooxPaypalPaymentController extends Controller { 
    ...
    public function index(){
        try {
            if(Auth::user()){
                ...
                return view('subscription.GdooxPaypalPayment.index', compact('paypalinfo'));
            }
            else {
    ...
    public function edit($id){
        try {
            if(Auth::user()){
                ...
                $paypalinfo = GdooxPaypalPayment::where('_id',$id)->first();

                return view('subscription.GdooxPaypalPayment.edit', compact('paypalinfo'));
            }
            else {
               return redirect('auth/login')->with('message',"Please Login!"); 
            }
  ...
  public function update($id) {
        try{
            if(Auth::user()){
                ...
                $data = GdooxPaypalPayment::where('_id','=',$id)->first();
                $data->paypal_name = $request['paypal_name'];
                $data->paypal_id = $request['paypal_id'];
               
                if($data->save()){
                    Session::flash('message', 'PayPal Info Updated Successfully');
                    return Redirect::route('gdoox-paypal.index');
                }
                else
                {
                    Session::flash('message', 'Subscription Info not updated ! Please Try Again');
                    return Redirect::route('gdoox-paypal.edit',$id)->with(Request::all());
                }
            }
            else {
