...
class UserReviewController extends Controller {
    ...
    public function index(){
        try {
            ...
            if(Auth::user()){
                ...
                $fm_data = DB::collection('field_master')->where('title', '=', 'review_form_info')->where('lang','=',$this->language)->first();
                $all_user_reviews = ProductReviews::where('userid','=',$userid)->paginate(25);
            }
            else {
                return redirect('auth/login')->with('message','You are not LoggedIn. Please Login');
            }

            return view('userreview.index',compact('fm_data','all_user_reviews','nav_menu','route'));
        }
        catch(\Exception $e){
    ...
    public function create(){
        try {
            ...
            $shopid = $data['shopid']; 
            return view('userreview.create', compact('fm_data','product_id','shopid'));
        }
        catch(\Exception $e){
             $errors = $this->errorMessage($e);
             return Redirect::route('exceptions.create')->with('message','Oops! You are here because Something went Wrong.  Please report the Error')->withErrors($errors);
    ...
    //        } else {
                // store
                $review = ProductReviews::where('_id', '=', $id)->first();
                $review->review_title = $data['review_title'];
                $review->review = $data['review'];
                $review->rating = (int)$data['rating'];
                $review->name = $data['name'];
