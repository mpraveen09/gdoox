...
function UGML_ShortCodeDetect() {
    ...
    $Pattern = get_shortcode_regex();
		...
		/** js */
			wp_enqueue_script( 'UGML-CHVPage-js', UGML_PLUGIN_URL.'js/CHVPage.js');
            wp_enqueue_script( 'UGML-DHVPage-js', UGML_PLUGIN_URL.'js/FWDUGP.js');		
}
add_action( 'wp_enqueue_scripts', 'UGML_ShortCodeDetect' );
add_filter( 'widget_text', 'do_shortcode' );

// Register Custom Post Type
add_action('init', 'PhotoGalleryPluginPro');
function PhotoGalleryPluginPro() {
...
class UGML {
	...
	private function __construct() {
        ...
        add_shortcode('ugmlgallery', array(&$this, 'shortcode'));
          add_filter( 'manage_edit-ugml_gallery_columns',  'ugml_gallery_columns' ) ;
    add_action( 'manage_ugml_gallery_posts_custom_column',  'ugml_gallery_manage_columns' );
        if (is_admin()) {
	...
	function ugml_gallery_columns( $columns ){
        $columns = array(
            'cb' => '<input type="checkbox" />',
            'title' => __( 'Ultimate Galleries','UGML_TEXT_DOMAIN' ),
            'shortcode' => __( 'Galleries Shortcode','UGML_TEXT_DOMAIN' ),
	...
	public function ugml_admin_print_scripts() {
		if ( 'ugml_cpt' == $GLOBALS['post_type'] ) {
			...
			wp_enqueue_script('jquery-ui-custom.min.js',UGML_PLUGIN_URL.'js/jquery-ui-custom.min.js', array('jquery'));
	...
	public function add_all_ugml_meta_boxes() {
		...
		add_meta_box(__('Pro Features', 'UGML_TEXT_DOMAIN') , __('Pro Features', 'UGML_TEXT_DOMAIN'), array(&$this,'ugml_pro_features'), 'ugml_cpt', 'side', 'low');
	}
	...
	?>
		<ul style="">
			<li class="plan-feature">(1) <?php _e('Responsive Design','UGML_TEXT_DOMAIN'); ?></li>
			...
			<li class="plan-feature">(4) <?php _e('Full multimedia support (image, self hosted video, youtube, vimeo, audio, flash, iframe, google maps, external link)','UGML_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(5) <?php _e('Super easy to use for beginners','UGML_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(6) <?php _e('All-purpose usage','UGML_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(7) <?php _e('Quick and easy setup','UGML_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(8) <?php _e('100% responsive','UGML_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(9) <?php _e('Mobile optimized','UGML_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(10) <?php _e('Light Box View','UGML_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(11) <?php _e('Single or multiple categories selection','UGML_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(12) <?php _e('Filterable categories','UGML_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(13) <?php _e('Optional menu','UGML_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(14) <?php _e('Customizable menu position with variation based on layout','UGML_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(15) <?php _e('Optional search box','UGML_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(16) <?php _e('Optional lazy loading with load more button or scroll','UGML_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(17) <?php _e('Multiple thumbnails hide / show animation types','UGML_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(18) <?php _e('Thumbnails multimedia icons','UGML_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(19) <?php _e('Adjustable thumbnails number to display / load per set','UGML_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(20) <?php _e('Adjustable thumbnail spacings / size and much more','UGML_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(21) <?php _e('Adjustable thumbnail geometry and styling','UGML_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(22) <?php _e('social networks sharing','UGML_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(23) <?php _e('Custom CSS Option','UGML_TEXT_DOMAIN'); ?></li>
			<li class="plan-feature">(24) <?php _e('And many more..','UGML_TEXT_DOMAIN'); ?></li>
		</ul>
	<?php
	}
    ...
    public function UGML_generate_add_image_meta_box_function($post) { ?>
		<div >
            ...
            <ul id="ugml_gallery_thumbs" class="clearfix">
				...
				//echo '<pre>'; print_r($WPGP_AllPhotosDetails); echo '</pre>';
				//die;
						...
						<li class="ugml-image-entry" id="ugml_img">
							...
							<div class="ugml-admin-inner-div2" >
								...
								<p>
									...
									<textarea id="UGML_image_descp[]" name="UGML_image_descp[]" class="ugml_textarea" placeholder="Enter Image Description"><?php echo esc_textarea($description); ?></textarea>
								</p>
								<p>
				...
				if($TotalImages) {
					for($i=0; $i < $TotalImages; $i++) {
						...
						$ImagesArray[] = array(
							...
							'UGML_image_descp' 				=> esc_textarea( $description ),
	...
	public function UGML_settings_meta_save($PostID) {
	  if(isset($PostID) && isset($_POST['UGML_useIconButtons'])){
		...
		$UGML_Settings_Array = serialize( array(
			...
			'UGML_otherThumbnail'          	=> $UGML_otherThumbnail,
...
function ugml_add_ugml_custom_button($context) {
  ...
  $container_id = 'UGML';
  $title =  __('Select Gallery to insert into post','UGML_TEXT_DOMAIN') ;
  $context = '<a class="button button-primary thickbox"  title="'. __("Select Gallery to insert into post",'UGML_TEXT_DOMAIN').'"
...
function ugml_add_ugml_inline_popup_content() {
	...
	<div id="UGML" style="display:none;">
	  <h3><?php _e('Select Any Gallery To Insert Into Post','UGML_TEXT_DOMAIN');?></h3>
	  <?php
