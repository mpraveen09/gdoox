...
class ImportNetworkProductsController extends Controller {
    ...
    public function viewPartnersProducts($site="", $networksite=""){
        if(Auth::user()){
            ...
            return view('backend.dashboard.business_partners.import_products.view_network_products',compact('list_products','site','networksite','fm_data','nav_menu','route'));
        }
        else {
    ...
    public function viewProducts($from_store="", $to_store=""){
        if(Auth::user()){
            ...
            $products = Products::whereNotIn('_id',$importedproducts)->where('shopid', $from_store)->where('product_type','!=','shared')->where('product_type','!=','opportunities')->where('product_type','!=','multi_item')->where('status','enabled')->get();      
