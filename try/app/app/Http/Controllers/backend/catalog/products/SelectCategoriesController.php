...
class SelectCategoriesController extends Controller {
    ...
    public function index(){
        try {
            ...
            $classifypost = DropdownOption::where('name','Classify Post')->first();
            if($classifypost){
            ...
            if(count($estores)>0){
                $categories= Categories::where('lang', '=', 'en')->Where('parent', '=', 0)->orderBy('name')->project( array('cat_id', 'name') )->get();
...
//            if(Request::ajax()){
                ...
                $categories_ = Categories::where('lang', '=', 'en')->Where('name', 'like', '%'.addslashes($input['term']).'%')->orderBy('name')->project( array('cat_id','name') )->get();
...
//              $cat_list = $this->category_tree($input['parent']);
        ...
        elseif($flag === TRUE) {
            $sql = Categories::where('lang', '=', 'en')
                ...
                if(count($sql)){
                    ...
                    $other_ids = Categories::where('lang', '=', 'en')
                        ->Where('parent', '=', $parent)
                        ->orderBy('name')
                        ->project(array('cat_id', 'name'))
                        ->get();
    ...
    public function checkchild($other_ids, $parent, $term, $flag, $user_tree_array){
        ...
        foreach($other_ids as $val){
            $level = Categories::where('lang', '=', 'en')
            ...
            if(count($level)){
                ...
                $user_tree_array[] = '</ul>';
                 ...
                 $other = Categories::where('lang', '=', 'en')
