...
class ShoppingCartController extends Controller {
    ...
    public function addBidToCart(){
        if(Auth::user()){
            $input = Request::all();
            $userid = Auth::user()->id;
            $user = User::where('_id','=',$userid)->first();
  
            $product = Products::find($input['_id']); 
            $shopid = $product->shopid;
            $cookie_val = $_COOKIE['gdoox_shopping_cart'];
        
            $find_product = ShoppingCart::where('product_id','=',$input['_id'])->where('cart_id','=',$cookie_val)->where('status','=','0')->first();
//            echo "<pre>";
//            print_r($input);
//            print_r($product);
//            exit;
            if(isset($find_product)){
                // Redirect directly to the next Route.
                // return redirect()->route('auth/login');
            }
            else {
                    $cart = new ShoppingCart();
                    $cart->product_id = $input['_id'];
                    $cart->part_number = (isset($product->product_data[4]) ? $product->product_data[4] : 'N/A');
                    $cart->seller_ref_code = (isset($product->product_data[308]) ? $product->product_data[308] : 'N/A');
                    ...
                    elseif(isset($product->product_data[20])){
                        $cart->price = $product->product_data[20];
                    }
                    elseif(isset($product->product_data[21])) {
                        $cart->price = $product->product_data[21];
                    }
                    elseif(isset($product->product_data[22])) {
                        $cart->price = $product->product_data[22];
                    }
                    ...
                    // Attr Id 14 (Auction Type) for Auction and 606 for Reverse Auction
                    if(isset($product->product_data[14])){
                        $proddata['14'] = $product->product_data[14];
                    }
                    elseif(isset($product->product_data[606])) {
                        $proddata['606'] = $product->product_data[606];
                    }
                    else {
                        $proddata['606'] = "";
                    }
                    ...
                    // Attr Id 20 (Auction Price) for Auction and 22 for Reverse Auction
                    if(isset($product->product_data[20])){
                        $proddata['20'] = $product->product_data[20];
                    }
                    elseif(isset($product->product_data[22])){
                        $proddata['22'] = $product->product_data[22];
                    }
                    
                    if(isset($product->product_data[21])){
                        $proddata['21'] = $product->product_data[21];
                    }

                    if(isset($product->product_data[23])){
                        $proddata['23'] = $product->product_data[23];
                    }
                    
                    if(isset($product->product_data[24])){
                        $proddata['24'] = $product->product_data[24];
                    }
                    
                    if(isset($product->product_data[25])){
                        $proddata['25'] = $product->product_data[25];
                    }
                    
                    if(isset($product->product_data[32])){
                        $proddata['32'] = $product->product_data[32];
                    }
                    
                    if(isset($product->product_data[33])){
                        $proddata['33'] = $product->product_data[33];
                    }
                    ...
                    $cart->viewed = '0';
                    $cart->cart_id = $cookie_val;
                    $cart->type = 'Auction Bid';
                    
                    if($cart->save()){
                        return Redirect::route('checkout.payment_proceed',['storeid' => $shopid]);
                    }
                    else {
...
//            }
    ...
    public function showWishList(){
        ...
        foreach ($wishlists as $wishlist) {
            $products = Products::whereIn('_id', $productsids)->project(array('desc','thumb','shopid','product_data', 'thumb_path'))->get();
        }

        return view('cart.wishlist',compact('products','nav_menu','route'));
    }
    ...
    public function abandonedCart(){
        if(Auth::user()){
            $abandoned_cart = $store_name = $user_name = $shopids = $userids = array();
