        ...
        @include('site.quick-menu')
            ...
            <div class="card">
                <div class="card-header card-padding-sm bgm-blue">
                    <h2><i class="zmdi zmdi-account m-r-5"></i> Companies Involved</h2>
                </div>   
                @if($involved_partners)
                  @foreach($involved_partners as $partners)
                      @foreach($partners as $partner)
                      <div class="card-body card-padding">
                          <table class="table table-striped responsive-utilities jambo_table ">
                            <tbody>
                                   <tr>
                                       <td><img style="height:50px; width: 50px" src="{{ asset('uploads/site_logo') }}/{{ $partner->company_logo }}" alt="" ></td>
                                       <td><h4>{!! $partner->company_name !!}</h4></td>         
                                   </tr>
                             </tbody>
                          </table>
                          </div>
